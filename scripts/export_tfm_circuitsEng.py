...
r"""
===============================================================================
TFM Circuit Exporter v2 — English
===============================================================================
Calls the real functions from qae_circuits.py to build the quantum circuits
used in the pipeline and exports them as PNG.

CHANGES v2:
- Deep decomposition to resolve opaque multiplexer_dg blocks
- Extra function that draws what multiplexer / multiplexer_dg is
- Dual export: high-level (blocks) + low-level (basis gates)

USAGE:
    Place this file in the same directory as qae_circuits.py and run:
        python export_tfm_circuitsEng.py

    PNGs are saved to: E:\\\\TFM\\\Codigo\results\circuits\
===============================================================================
"""
...
# How many decompose() levels to apply to reach basis gates
# 3 = what your original code does (leaves multiplexer_dg)
# 6-8 = decomposes down to Ry, CX, X, etc.
DECOMPOSE_DEPTH = 8
...
print("=" * 70)
print("TFM CIRCUIT EXPORTER v2 — English")
...
def deep_decompose(qc, depth=DECOMPOSE_DEPTH):
    """Iteratively decompose a circuit down to basis gates."""
    result = qc
...
def has_opaque_gates(qc):
    """Check if the circuit still contains opaque blocks."""
    opaque = set()
    for inst in qc.data:
...
# ─────────────────────────────────────────────────────────────────────────────
# EXTRA: MULTIPLEXER_DG EXPLANATION DIAGRAM
# ─────────────────────────────────────────────────────────────────────────────

def draw_multiplexer_explanation():
    """
    Draws a diagram explaining what multiplexer / multiplexer_dg is:
    a uniformly controlled Ry rotation (Moettonen decomposition).

    multiplexer    = applies Ry(alpha_i) conditioned on control state |i>
    multiplexer_dg = its adjoint (dagger)

    Decomposes into an alternating sequence of Ry and CNOT (Gray code ordering).
    """
    ...
    ax.set_title("(a) High-level block", fontsize=12,
                 fontfamily="serif", fontweight="bold", pad=10)

    # ── Right panel: basis gate decomposition ──
    ax = ax_decomp
    ax.set_xlim(-1, 16)
    ax.set_ylim(-1.5, 4.5)
    ax.set_aspect("equal")
    ...
    ax.text(7.5, -0.7,
            ...
            "(Gray code ordering, Ref: Möttonen et al. 2004)",
            fontsize=10, fontfamily="serif", ha="center", va="center",
            color="#6B7280", fontstyle="italic")

    ax.set_title("(b) Decomposition into basis gates",
                 fontsize=12, fontfamily="serif", fontweight="bold", pad=10)

    fig.suptitle(
        r"What is multiplexer_dg? — Uniformly Controlled $R_y$ Rotation",
...
def main():
    print("\n[1] Generating portfolio data...")
    returns, weights, losses = generate_data()
    var_threshold = float(np.percentile(losses, (1 - ALPHA) * 100))
    print(f"    {returns.shape[0]} periods x {returns.shape[1]} assets")
    print(f"    VaR (95th percentile): {var_threshold:.6f}")
    ...
    # ── 0: Multiplexer_dg explanation ──
    print("\n[2] Multiplexer_dg explanation diagram...")
    ...
    save_png(qc_dist, "circuit_1a_state_prep_blocks.png",
             title=r"$A_{\mathrm{load}}$: State Preparation (Möttonen) — "
                   f"high level (n={n}, N={2**n})")
    qc_dist_decomp = deep_decompose(qc_dist)
    save_png(qc_dist_decomp, "circuit_1b_state_prep_gates.png",
             title=r"$A_{\mathrm{load}}$: State Preparation — basis gates",
             fold=80, scale=0.5)
    ...
    # ── 3: Tail Probability ──
    print("\n[5] Circuit 3: Tail Probability (Operator A)...")
    ...
    save_png(qc_tail, "circuit_3a_tail_prob_blocks.png",
             title=r"Operator $\mathcal{A}$: $P(L(\mathbf{w}) \geq z)$ — "
                   "high level")
    ...
    save_png(grover_draw.decompose(), "circuit_4a_grover_blocks.png",
             title=r"Grover Operator $Q = -\mathcal{A}\,S_0\,"
                   r"\mathcal{A}^{-1}\,S_\chi$ — high level",
             fold=60, scale=0.6)
    grover_decomp = deep_decompose(grover_draw)
    save_png(grover_decomp, "circuit_4b_grover_gates.png",
             title=r"Grover Operator $Q$ — basis gates",
             fold=120, scale=0.35)

    # ── 5: Subgradient Component ──
    print("\n[7] Circuit 5: Subgradient Component A_j (j=0)...")
    ...
    if qc_cond is not None:
        save_png(qc_cond, "circuit_5a_subgradient_blocks.png",
                 title=r"$\mathcal{A}_0$: Subgradient $g_0(\mathbf{w}) = "
                       r"-E[r_0 \mid L \geq \mathrm{VaR}_\alpha]$ — high level")
        qc_cond_decomp = deep_decompose(qc_cond)
        save_png(qc_cond_decomp, "circuit_5b_subgradient_gates.png",
                 title=r"$\mathcal{A}_0$: Subgradient Component — basis gates",
                 fold=100, scale=0.4)
    ...
    save_png(qc_twl, "circuit_6a_tail_weighted_blocks.png",
             title=r"$E[L \cdot \mathbb{1}\{L \geq \mathrm{VaR}\}]$ — "
                   f"high level (tail bins={twl_meta.get('tail_levels_count','?')})")
    ...
    print(f"{len(pngs)} PNGs exported to: {OUTPUT_DIR}\n")
    for p in pngs:
        print(f"  {p.name:50s} ({p.stat().st_size/1024:6.0f} KB)")
    print("\n" + "=" * 70)
    print("\nFOR THE TFM:")
    print("  Body     -> *_blocks.png  (clean, readable block diagrams)")
    print("  Appendix -> *_gates.png   (full depth, Ry/CX/X basis gates)")
    print("  Legend   -> circuit_0_multiplexer_explanation.png")
