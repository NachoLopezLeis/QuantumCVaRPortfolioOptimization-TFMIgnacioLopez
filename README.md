...
### v10 additions — ZNE comparison and ε=0.001 precision point

```bash
# Comparative ZNE in the qiskit-aer IQM noise model (no hardware, no mitiq).
# raw / global fold / gate fold / identity insertion (FIIM), E1/E2/E3, lambda={1,3,5}.
python scripts/hw_sim_zne_compare.py            # -> results/hardware_validation/sim_zne_compare_latest.json
python scripts/fig_zne_compare.py               # -> results/paper_figures/lopez8b.png
python scripts/gen_zne_tables.py                # -> results/paper_tables/tab_zne_compare.tex

# Fourth precision point eps=0.001 (needs data/returns_sp500_full.csv)
python scripts/run_single_experiment.py config_A4_PA.yaml
```

Noise-scaling primitives live in `src/phase_2/zne_foldingfree.py`
(`global_fold`, `gate_fold`, `identity_insertion`, and the Richardson / linear /
poly2 / exponential extrapolators). The experiment is deterministic (seed 42).
